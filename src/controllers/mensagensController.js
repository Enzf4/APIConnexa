  ...
  try {
    ...
    res.json({
      mensagens: mensagens.reverse().map((msg) => ({
        ...msg,
        usuario_avatar: msg.usuario_avatar || AVATAR_DEFAULT,
...
const enviarMensagem = async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      mensagem: {
        ...mensagem,
        usuario_avatar: mensagem.usuario_avatar || AVATAR_DEFAULT,
      },
    });
  } catch (error) {
...
const deletarMensagem = async (req, res) => {
  try {
    ...
    const podeDeletar =
      mensagem.usuario_id === userId || participante.papel === "admin";
    ...
    // Deletar mensagem
    await runQuery("DELETE FROM mensagens WHERE id = ?", [mensagemId]);
