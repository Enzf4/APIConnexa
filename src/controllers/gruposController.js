...
const { AVATAR_DEFAULT } = require("../utils/constants");
...
const criarGrupo = async (req, res) => {
  try {
    ...
    const grupo = await getQuery(
      `SELECT g.*, u.nome as criador_nome, u.avatar as criador_avatar, u.foto_perfil as criador_foto
             FROM grupos g 
...
const buscarGrupos = async (req, res) => {
  try {
    ...
    let whereConditions = [
      "g.ativo = TRUE",
      "g.participantes_atual < g.limite_participantes",
    ];
    let params = [];
    ...
    res.json({
      grupos: grupos.map((grupo) => ({
        ...
        criador: {
          nome: grupo.criador_nome,
          curso: grupo.criador_curso,
          periodo: grupo.criador_periodo,
          avatar: grupo.criador_avatar || AVATAR_DEFAULT,
          foto_perfil: grupo.criador_foto,
        },
      })),
  ...
  try {
    ...
    if (participante.papel === "admin") {
      return res.status(400).json({
        ...
        message:
          "O criador do grupo não pode sair. Transfira a administração ou delete o grupo",
      });
    }
    ...
    const grupo = await getQuery(
      "SELECT nome, criador_id FROM grupos WHERE id = ?",
      [grupoId]
    );
    const usuario = await getQuery("SELECT nome FROM usuarios WHERE id = ?", [
      userId,
    ]);
  ...
  try {
    ...
    if (!grupo) {
      return res.status(404).json({
        error: "Grupo não encontrado",
        message:
          "Grupo não encontrado ou você não tem permissão para deletá-lo",
      });
    }

    // Marcar grupo como inativo
    await runQuery("UPDATE grupos SET ativo = FALSE WHERE id = ?", [grupoId]);
